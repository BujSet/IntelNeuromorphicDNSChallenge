...
fig, axs = plt.subplots(1, 5, sharex=True, sharey=True, figsize=(20, 10))
...
im = axs[0].imshow(heatmaps[0], vmin=vLow, vmax=vHigh, cmap='autumn', interpolation='nearest', aspect='auto', origin='lower')
im = axs[1].imshow(heatmaps[1], vmin=vLow, vmax=vHigh, cmap='autumn', interpolation='nearest', aspect='auto', origin='lower')
im = axs[2].imshow(heatmaps[2], vmin=vLow, vmax=vHigh, cmap='autumn', interpolation='nearest', aspect='auto', origin='lower')
im = axs[3].imshow(heatmaps[3], vmin=vLow, vmax=vHigh, cmap='autumn', interpolation='nearest', aspect='auto', origin='lower')
im = axs[4].imshow(heatmaps[4], vmin=vLow, vmax=vHigh, cmap='autumn', interpolation='nearest', aspect='auto', origin='lower')
...
axs[2].set_xlabel("Vertical Elevation", fontweight="bold", fontsize=16, y=0.10)
axs[0].set_ylabel("Right Ear\nInput Frequency (kHz)", fontweight="bold", fontsize=16)
# axs[1,0].set_ylabel("Channel 0\nInput Frequency (kHz)", fontweight="bold", fontsize=16)
axs[0].set_xticks([0,24,48])
axs[0].set_xticklabels(["Front","Up","Back"])
myyticks = list(range(0, len(tones), len(tones)//4))
axs[0].set_yticks(myyticks)
axs[0].set_yticklabels([str(round(tones[i]/1000.0, 1)) for i in myyticks])
# posits = CipicDatabase.subjects[SUBJECT].getCartesianPositions()
...
axs[1].set_title("Middle-Left") 
axs[2].set_title("Middle") 
axs[3].set_title("Middle-Right")
axs[4].set_title("Right")
# azimuth = round(CipicDatabase.subjects[SUBJECT].getSphericalPositionsFromIndex(1208)[0], 1)
...
plt.savefig("magnitude.png", bbox_inches='tight', format="png")
plt.close()
