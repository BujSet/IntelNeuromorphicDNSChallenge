...
#    writer = SummaryWriter('runs/' + identifier)
    ...
    if (args.ssnns):
        ...
        speechFilter0  = torch.from_numpy(CIPICSubject.getHRIRFromIndex(args.speechFilterOrient, 0)).float()
        speechFilter0  = speechFilter0.to(device)
        speechFilter1  = torch.from_numpy(CIPICSubject.getHRIRFromIndex(args.speechFilterOrient, 1)).float()
        speechFilter1  = speechFilter1.to(device)
        noiseFilter0   = torch.from_numpy(CIPICSubject.getHRIRFromIndex(args.noiseFilterOrient, 0)).float()
        noiseFilter0   = noiseFilter0.to(device)
        noiseFilter1   = torch.from_numpy(CIPICSubject.getHRIRFromIndex(args.noiseFilterOrient, 1)).float()
        noiseFilter1   = noiseFilter1.to(device) 
        ...
        for i, (noisy, clean, noise, idx) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                if (args.ssnns):
                    ...
                    ssl_clean0 = torch.zeros(args.b, 480000).to(device)
                    ssl_clean1 = torch.zeros(args.b, 480000).to(device)
                    ...
                    for batch_idx in range(args.b):
                        if (args.randomize_orients):
                            ...
                            speechFilter0  = torch.from_numpy(CIPICSubject.getHRIRFromIndex(speechOrient, 0)).float()
                            speechFilter0  = speechFilter0.to(device)
                            speechFilter1  = torch.from_numpy(CIPICSubject.getHRIRFromIndex(speechOrient, 1)).float()
                            speechFilter1  = speechFilter1.to(device)
                            noiseFilter0   = torch.from_numpy(CIPICSubject.getHRIRFromIndex(noiseOrient, 0)).float()
                            noiseFilter0   = noiseFilter0.to(device)
                            noiseFilter1   = torch.from_numpy(CIPICSubject.getHRIRFromIndex(noiseOrient, 1)).float()
                            noiseFilter1   = noiseFilter1.to(device)
                        ssl_noise0[batch_idx,:] = conv_transform(noise[batch_idx,:], downsampler(noiseFilter0))
                        ssl_noise1[batch_idx,:] = conv_transform(noise[batch_idx,:], downsampler(noiseFilter1))
                        ssl_clean0[batch_idx,:] = conv_transform(clean[batch_idx,:], downsampler(speechFilter0))
                        ssl_clean1[batch_idx,:] = conv_transform(clean[batch_idx,:], downsampler(speechFilter1))
                    ...
                    ssl_noisy0, ssl_clean0, ssl_noise0 = module.synthesizeNoisySpeech(
                        ssl_clean0, 
                        ssl_noise0, 
                        args.b, 
                        ssl_snrs,
                    ...
                    ssl_noisy1, ssl_clean1, ssl_noise1 = module.synthesizeNoisySpeech(
                        ssl_clean1, 
                        ssl_noise1, 
                        args.b, 
                        ssl_snrs,
            ...
            if (args.spectrogram == 0):
                noisy_abs0, noisy_arg0 = stft_splitter(ssl_noisy0, args.n_fft, None)
                noisy_abs1, noisy_arg1 = stft_splitter(ssl_noisy1, args.n_fft, None)
                clean_abs0, clean_arg0 = stft_splitter(ssl_clean0, args.n_fft, None)
                clean_abs1, clean_arg1 = stft_splitter(ssl_clean1, args.n_fft, None)
            elif(args.spectrogram == 1):
                noisy_abs0, noisy_arg0 = stft_splitter(ssl_noisy0, args.n_fft, stft_transform)
                noisy_abs1, noisy_arg1 = stft_splitter(ssl_noisy1, args.n_fft, stft_transform)
                clean_abs0, clean_arg0 = stft_splitter(ssl_clean0, args.n_fft, stft_transform)
                clean_abs1, clean_arg1 = stft_splitter(ssl_clean1, args.n_fft, stft_transform)
            else:
